...
f1 = open('/Users/mac/spader/resource/jd_download_items.csv', 'a+')
writer = csv.writer(f1)  #创建写的对象
...
def get_item_html_text(url):
    ...
    try:
        ...
        #获取数据
        image= 'https:{}'.format(soup.find("div", id="spec-n1").find('img').get('data-origin'))

        brand = '小米'
        title = soup.find('div', class_='sku-name').get_text().strip()
        print(title)
        print(image)
        # price = soup.find(
        #     'div', class_='summary-price J-summary-price')
        # print(price)
        # size 没有开发好
...
def query_items():
    # lists = []
    for item in open('/Users/mac/spader/resource/jd_upload_item.txt', encoding='utf-8').readlines():  # 逐行读取url文件
        print(item)
        if item:
            try:
                goods = get_item_html_text(item)
                print(goods)
                lists.append(goods)
                time.sleep(5)
            except:
